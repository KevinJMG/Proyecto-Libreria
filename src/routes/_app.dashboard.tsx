...
function DashboardPage() {
  ...
  const top = [...books].sort((a, b) => b.popularity - a.popularity).slice(0, 4);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <section className="relative overflow-hidden rounded-2xl bg-gradient-brand p-6 text-white shadow-brand sm:p-8">
        ...
        <p className="text-sm/6 uppercase tracking-widest text-white/70">Bienvenido de nuevo</p>
        <h1 className="mt-1 text-3xl font-bold sm:text-4xl">Hola, {user?.name?.split(" ")[0]}</h1>
        <p className="mt-2 max-w-lg text-white/80">
          Explora el catálogo, gestiona tus préstamos y descubre nuevas lecturas.
        </p>
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard icon={Library} label="Libros en catálogo" value={books.length} />
        <StatCard icon={BookOpenCheck} label="Disponibles" value={books.filter((b) => b.available).length} />
        <StatCard icon={BookMarked} label="Préstamos activos" value={activeMine} />
        <StatCard icon={TrendingUp} label="Vencidos" value={overdueMine} tone={overdueMine ? "danger" : undefined} />
      </section>
            ...
            <Card key={b.id} className="overflow-hidden py-0">
              ...
              <CardContent className="p-3">
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-muted-foreground">★ {b.popularity}</span>
                </div>
              </CardContent>
            </Card>
          ))}
